...
class SV:
    ...
    def same_variant(self, other):
        return (self.type, self.ref_chrom, self.ref_start, self.ref_end, self.copynumber, self.bp1, self.bp1) == (other.type, other.ref_chrom, other.ref_start, other.ref_end, other.copynumber, other.bp1, other.bp1)
    ...
    def __repr__(self):
        string = '({0}, {1}, {2}, {3}, len = {4}, CN = {5}, BP = {6}:{7}, GT = {8}, ID = {9}, OR = {10}, INS = {11})'
...
def classify_svs(path, blocks, num_genome_blocks, left_bp, right_bp, verbosity):
    ...
    while i < len(breakpoints):
        ...
        if not (inverted_before or inverted_after) and \
            ...
            split_support = supporting_read_count(splits1, splits2, 'Del')
            pe_support = supporting_read_count(pe1, pe2, 'Del')
        ...
        if j > bp + 1:
            ...
            if is_decreasing(block_sequence[bp + 1 : j]) and \
                ...
                split_support = supporting_read_count(splits1, splits2, 'InvL') + \
                                supporting_read_count(splits1, splits2, 'InvR')
                pe_support = supporting_read_count(pe1, pe2, 'InvL') + \
                             supporting_read_count(pe1, pe2, 'InvR')
                svs.append(SV(sv_type, sv_chrom, sv_start, sv_end, sv_length,
                              sv_copynumber, bp1 = bp1, bp2 = bp2,
                              split_support = split_support,
                              pe_support = pe_support))
        ...
        if dup_count > 1:
            ...
            if found:
                ...
                is_duplicated = (dup_sequence == list(range(first_dup_block, last_dup_block + 1)) * dup_count)
                is_properly_oriented = not any(inverted[j:min(len(block_sequence), idx_afterdup)])
                if block_afterdup_ok and block_beforedup_ok and is_duplicated and is_properly_oriented:
                    ...
                    all_bps = [get_bp_interval(blocks, i, '+', num_genome_blocks) for i in range(first_dup_block,
                                                                                                  last_dup_block)]
                    all_bps.append(bp2)
                    ...
                    split_support = supporting_read_count(splits1, splits2, 'Dup')
                    pe_support = supporting_read_count(pe1, pe2, 'Dup')
        ...
        if blocks[block_after].is_insertion():
            ...
            if inverted_after:
                ...
                splits2 = left_bp[block_after].splits
                pe2 = left_bp[block_after].pe
            split_support = supporting_read_count(splits1, splits2, expected_split_type)
            pe_support = supporting_read_count(pe1, pe2, expected_split_type)
...
def supporting_read_count(r1, r2, expected_type):
